...
from tca_web.domain.income_tracking import (
    SECONDS_PER_DAY,
...
def _ts(day, hour=18, minute=10):
    return int(datetime.datetime(2026, 8, day, hour, minute, tzinfo=datetime.UTC).timestamp())
...
def test_summary_allocates_star_ranges_by_income_rank_slots():
    ...
    summaries = {row["stars"]: row for row in summarize_star_ranges(totals, _ts(2))}
